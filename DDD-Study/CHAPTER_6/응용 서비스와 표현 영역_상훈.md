...
- 필요한 경우
  - 구현 클래스가 여러개인 경우 혹은 런타임 시 <u>구현 객체를 교체해야 하는 경우</u>
  ...
  표현 영역의 단위 테스트를 위해 <u>응용 서비스 클래스의 가짜 객체가 필요한데 이를 위해 인터페이스 추가</u>.
##### 다. 메서드 파라미터와 값 리턴
...
- 권한 검사 기능 구현 위치
  ...
  - 도메인
    ...
    - 직접 권한 검사 로직을 구현해야 함
- 보안 프레임워크에 대한 이해도가 높지 않은 경우 도메인에 맞는 권한 검사ㅏ 기능을 직접 구현하는 것이 코드 유지 보수에 유리.
### 7. 조회 전용 기능과 응용 서비스
